...
export default function ConnectPage() {
  ...
  const { ref: feedRef, isVisible: feedVisible } = useScrollReveal(0.1);
  ...
  // Initialize undo stack with blank canvas state
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;
    // Push the blank state so first undo brings back a clean canvas
  ...
  const undo = useCallback(() => {
    ...
    const ctx = canvas.getContext("2d");
    if (!ctx) return;
    // Need at least a saved state + a previous state to restore to
    if (undoStackRef.current.length <= 1) return;

    // Pop the current state from the stack
    undoStackRef.current.pop()!;
    // Restore the state before it (the new top of the stack)
  ...
  // Map pointer coordinates to canvas coordinate space
  ...
  return (
    <PageTransition>
      <main className="mx-auto flex w-full max-w-3xl flex-col bg-white px-6 sm:px-10 md:px-10 pt-6 sm:pt-16 md:pt-20 pb-22 sm:pb-8 min-h-screen">
        ...
        <div
          ref={formRef}
          className={`mt-8 flex flex-col gap-4 transition-all duration-700 ease-out ${
            formVisible
              ? "opacity-100 translate-y-0"
              : "opacity-0 translate-y-6"
          }`}
            ...
            <div className="flex flex-col border-[3px] border-black rounded-xl overflow-hidden relative">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 border-b-[3px] border-black bg-gray-50 px-4 py-2 text-xs font-bold">
                ...
                <div className="flex items-center gap-2">
                  {/* Mobile draw mode toggle — only visible on small screens */}
                  <button
                    type="button"
                  ...
                  >
                    {drawMode ? 'Click to Scroll' : 'Click to Draw'}
                  </button>
                  <button
                    type="button"
              ...
              <div className="flex justify-between items-center border-t-[3px] border-black p-2 bg-white">
                <div className="flex items-center gap-2">
                  ...
                  <button 
                    type="button" 
                    onClick={undo}
                    disabled={submitting || undoStackRef.current.length <= 1}
        ...
        <div
          ref={feedRef}
          className={`mt-12 flex flex-col gap-6 transition-all duration-700 ease-out ${
            feedVisible
              ? "opacity-100 translate-y-0"
              : "opacity-0 translate-y-6"
          }`}
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 stagger-children">
              {submissions.map((sub, index) => (
                <div
                  key={sub.id}
                  className="border-[3px] border-black bg-white p-4 flex flex-col gap-3 rounded-xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] animate-fade-in-up"
